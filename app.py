...
ROOMS_FILE = os.path.join(BASE_DIR, 'rooms.json') # Ensure rooms.json is in the same directory
try:
    with open(ROOMS_FILE, 'r') as f:
        AVAILABLE_ROOMS = json.load(f)
except FileNotFoundError:
    st.error(f"Error: {ROOMS_FILE} not found. Please ensure it exists in the application directory.")
    AVAILABLE_ROOMS = [] # Default to empty list if file not found

oasis = next((r for r in AVAILABLE_ROOMS if r["name"] == "Oasis"), {"capacity": 15}) # Default capacity if Oasis not found or file error
...
def get_room_grid(pool):
    ...
    # OFFICE_TIMEZONE = pytz.timezone("Europe/Amsterdam") # This is defined globally, avoid redefining
    ...
    try:
        with open(ROOMS_FILE) as f: # Use ROOMS_FILE constant
            all_rooms = [r["name"] for r in json.load(f) if r["name"] != "Oasis"]
    except FileNotFoundError:
        st.error(f"Error: {ROOMS_FILE} not found when trying to get room grid.")
        return pd.DataFrame()
    except json.JSONDecodeError:
        st.error(f"Error: Could not decode {ROOMS_FILE}.")
        return pd.DataFrame()
    ...
    if not conn: return pd.DataFrame(grid.values()) # Return empty grid if no connection
    try:
        ...
        for row in allocations:
            team = row["team_name"]
            room = row["room_name"]
            date = row["date"] # This is already a date object from the DB
        ...
        return pd.DataFrame(grid.values()) # Return current grid on error
    finally:
...
def get_oasis_grid(pool):
    ...
    try:
        with conn.cursor() as cur:
            ...
            all_days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"] # Added Friday for Oasis
...
def get_oasis_preferences(pool):
    ...
    try:
        with conn.cursor() as cur:
            # Adjusted to fetch up to 5 preferred days if your schema supports it
            cur.execute("""
...
def insert_preference(pool, team, contact, size, days):
    ...
    if size < 3:
        ...
        return False
    if size > 6: # Assuming max size is 6 based on your description; adjust if needed
    ...
    try:
        with conn.cursor() as cur:
            ...
            # This logic was in your original code, kept for consistency
            new_days_set = set(days.split(','))
            if not (new_days_set == {"Monday", "Wednesday"} or new_days_set == {"Tuesday", "Thursday"}):
            ...
            """, (team, contact, size, days)) # Store submission_time in UTC
            conn.commit()
            return True
    except psycopg2.Error as e:
...
def insert_oasis(pool, person, selected_days): # Modified to take list of days
    if not pool: return False
    if not person:
    ...
    try:
        with conn.cursor() as cur:
            ...
            """, (person.strip(), *padded_days)) # Store submission_time in UTC
            conn.commit()
            return True
    except psycopg2.Error as e:
...
# --- UI ---
st.title("📅 Weekly Room Allocator") # Changed
st.info("""
💡 **How This Works:**
...
if pool: # Only show UI if pool is available
    # --- Admin ---
    with st.expander("🔐 Admin Controls"):
        ...
        if pwd == RESET_PASSWORD:
            ...
            if st.button("🚀 Run Project Room Allocation"):
                if run_allocation: # Check if function is available
                    ...
                    if success: st.success("✅ Project room allocation completed.")
            ...
            if st.button("🎲 Run Oasis Allocation"):
                if run_allocation:
                    ...
                    if success: st.success("✅ Oasis allocation completed.")
                    ...
                    if conn_admin_tp:
                        try:
                            with conn_admin_tp.cursor() as cur:
                                cur.execute("DELETE FROM weekly_preferences") # Clear before re-inserting
            ...
            df_oasis_prefs_admin = get_oasis_preferences(pool) # Fetches up to 5 days
            if not df_oasis_prefs_admin.empty:
                # Ensure columns match what get_oasis_preferences returns
                        ...
                        try:
                            with conn_admin_op.cursor() as cur:
                                ...
                                for _, row in editable_oasis_df.iterrows():
                                    # Handle potentially missing 'Submitted At' if it's not always there or editable
        ...
        elif pwd: # Only show error if password was entered
            st.error("❌ Incorrect password.")

    # --- Team Form ---
    st.header("📝 Request Project Room") # Changed
    # The dates here are examples based on your original text. You might want to make these dynamic.
    st.markdown("For teams of 3 or more. Submissions for the **week of 2 June** are open from **Wednesday 28 May 09:00** until **Thursday 29 May 16:00**.")
    with st.form("team_form"):
        team_name = st.text_input("Team Name")
        contact_person = st.text_input("Contact Person")
        team_size = st.number_input("Team Size (3-6)", min_value=3, max_value=6, value=3) # Adjusted min/max
    ...
    # --- Oasis Form ---
    st.header("🌿 Reserve Oasis Seat") # Changed
    st.markdown("Submit your personal preferences for the Oasis for the **week of 2 June**. Submissions open from **Wednesday 28 May 09:00** until **Friday 30 May 16:00**.")
    ...
    # --- Add Yourself to Oasis (Ad-hoc) ---
    st.header("🚶 Add Yourself to Oasis (Ad-hoc)") # Changed
    st.caption("Use this to add yourself to the Oasis schedule if you missed the preference submission. Subject to availability.")
                ...
                if not conn_adhoc: st.error("No DB Connection");
                    ...
                    try:
                        with conn_adhoc.cursor() as cur:
                            ...
                            # First, remove any existing entries for this person on the selected days for this week to avoid duplicates if they re-add
                            ...
                            for day_str in adhoc_oasis_days:
                                ...
                                if count >= oasis.get("capacity", 15): # Use .get for safety
                            ...
                            elif adhoc_oasis_days: # Some warning might have occurred
    ...
    # --- Full Weekly Oasis Overview (Matrix) ---
    ...
    if not conn_matrix: st.error("No DB connection for Oasis Overview");
        ...
        try:
            with conn_matrix.cursor() as cur:
                cur.execute("SELECT team_name, date FROM weekly_allocations WHERE room_name = 'Oasis' AND date >= %s AND date <= %s",
                            (oasis_overview_monday, oasis_overview_days_dates[-1]))
                rows = cur.fetchall()

            df_matrix = pd.DataFrame(rows, columns=["Name", "Date"])
            if not df_matrix.empty:
                df_matrix["Date"] = pd.to_datetime(df_matrix["Date"]).dt.date

            # Get all unique names from allocations for the week, plus Niek
            # Also consider names from oasis_preferences for this week if you want them in the matrix even if not allocated yet
            unique_names_allocated = set(df_matrix["Name"]) if not df_matrix.empty else set()
            
            # Fetch names from oasis_preferences to ensure they appear in the matrix
            # This part is optional but makes the matrix more comprehensive
            ...
            except psycopg2.Error: # Catch DB error specifically
            ...
            if not all_relevant_names: all_relevant_names = ["Niek"] # Ensure Niek is there if no one else
            ...
            if not df_matrix.empty:
                for idx, row_data in df_matrix.iterrows():
                    ...
                    if alloc_date in oasis_overview_days_dates:
                        ...
                        if person_name in matrix_df.index: # Ensure person is in index
                             matrix_df.at[person_name, day_label] = True
            
            # Niek is always signed up
            for day_n in oasis_overview_day_names:
                if "Niek" in matrix_df.index:
                    matrix_df.at["Niek", day_n] = True
                elif "Niek" not in matrix_df.index and "Niek" in all_relevant_names: # Should not happen if logic above is correct
                    matrix_df.loc["Niek"] = [True] * len(oasis_overview_day_names)


            st.subheader("🪑 Oasis Availability Summary")
            # Calculate used spots based on the matrix_df which reflects current allocations for the week
            for day_dt, day_str_label in zip(oasis_overview_days_dates, oasis_overview_day_names):
                ...
                # Niek's special handling: if Niek is marked True in matrix but not in raw data, he still takes a spot.
                # However, the matrix_df is built from allocations, so Niek being True there means he's in allocations.
                # This count should be accurate based on the matrix source.
            ...
            edited_matrix = st.data_editor(
                ...
                disabled=["Niek"] if "Niek" in matrix_df.index else [], # Disable Niek's row editing
            ...
            if st.button("💾 Save Oasis Matrix Changes"):
                try:
                    with conn_matrix.cursor() as cur:
                        # Clear existing Oasis allocations for the current week, except for Niek
                        cur.execute("""
                            DELETE FROM weekly_allocations 
                            WHERE room_name = 'Oasis' AND team_name != 'Niek'
                            AND date >= %s AND date <= %s
                        """, (oasis_overview_monday, oasis_overview_days_dates[-1]))

                        # For Niek, ensure he's allocated if matrix says so (should always be true)
                        # This re-adds Niek based on matrix state, ensuring consistency.
                        ...
                        # Process other users
                        occupied_counts_per_day = {day_col: (1 if "Niek" in edited_matrix.index and edited_matrix.at["Niek", day_col] else 0) 
                        ...
                        for person_name_matrix in edited_matrix.index:
                            if person_name_matrix == "Niek":
                                continue
                            for day_idx, day_str_col in enumerate(oasis_overview_day_names):
                                if edited_matrix.at[person_name_matrix, day_str_col]: # If checked in the matrix
